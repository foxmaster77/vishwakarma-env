...
def test_maintenance_call_costs_more_when_machine_offline(env):
    env.reset()
    # Take a machine offline first, then call maintenance
    env.state.machine_online["cnc_mill"] = False
    env.state.maintenance_eta["cnc_mill"] = 3
    obs_maint = env.step(VishwakarmaAction(
        directive="call_maintenance",
        call_maintenance=True
    ))
    env2 = VishwakarmaEnvironment(factory_id="auto_components_pune", seed=42)
    env2.reset()
    obs_normal = env2.step(VishwakarmaAction(directive="run_normal"))
    assert obs_maint.cost_today_INR > obs_normal.cost_today_INR
